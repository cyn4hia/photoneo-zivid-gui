...
def main() -> int:
    ...
    reader_code = (
        ...
        "sys.stdout.write('READER: alive, press keys here\\n')\n"
        "sys.stdout.flush()\n"
        "while True:\n"
        "    try:\n"
        "        ch = msvcrt.getwch()\n"
        "    except Exception as e:\n"
        "        sys.stdout.write('READER: getwch failed: ' + str(e) + '\\n')\n"
        "        sys.stdout.flush()\n"
        "        continue\n"
        "    sys.stdout.write(repr(ch) + '\\n')\n"
        "    sys.stdout.flush()\n"
        "    if ch.lower() == 'q':\n"
        "        sys.stdout.write('READER: q pressed, but staying alive\\n')\n"
        "        sys.stdout.flush()\n"
    )
    ...
    def key_reader():
        ...
        while not stop_reader.is_set():
            ...
            if not line:
                print("[trace] !! reader subprocess stdout closed (child died?)", flush=True)
                time.sleep(0.1)
                continue
            line = line.strip()
            ...
            try:
                ...
                print(f"[trace] couldn't parse reader output {line!r}: {e}", flush=True)
                continue
